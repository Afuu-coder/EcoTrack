...
export default function InsightsSection({ insights, loadingAI }) {
  ...
  if (loadingAI) {
    return (
      <section aria-labelledby="insights-heading">
        <h2 id="insights-heading" className="section-title font-display" style={{ fontSize: 18 }}>AI-Powered Insights</h2>
        <div role="status" aria-live="polite" className="glass-panel" style={{ textAlign: 'center', padding: 48 }}>
          <div style={{ fontSize: 36, marginBottom: 14, animation: 'pulse-ring 2s ease infinite' }} aria-hidden="true">🤖</div>
          <p style={{ color: 'var(--text-secondary)', margin: '0 0 16px' }}>
            Calling <strong style={{ color: 'var(--accent-purple)' }}>Vertex AI</strong> for personalised recommendations…
          </p>
          <div className="shimmer" style={{ background: 'linear-gradient(90deg, var(--accent-purple), var(--accent-blue), var(--accent-purple))', backgroundSize: '200%' }} />
        </div>
      </section>
    );
  }
  ...
  return (
    <section aria-labelledby="insights-heading">
      ...
      {/* Summary */}
      <div className="glass-panel" style={{ padding: '24px', marginBottom: '24px', background: 'rgba(139, 92, 246, 0.08)', borderColor: 'rgba(139, 92, 246, 0.2)' }}>
        <p style={{ margin: 0, fontSize: 15, color: 'var(--text-secondary)', lineHeight: 1.6 }}>
          Your biggest impact area is{' '}
          <strong style={{ color: 'var(--accent-purple)', textTransform: 'capitalize' }}>{insights.largestCategory}</strong>.
          By acting on the tips below, you could save up to{' '}
          <strong style={{ color: 'var(--accent-emerald)' }}>{insights.potentialSaving} kg CO₂e/month</strong>
          {' '}— roughly a <strong>{Math.round((insights.potentialSaving / (insights.total || insights.potentialSaving + 1)) * 100)}%</strong> reduction.
        </p>
      </div>
      ...
      {/* 30-day Challenge */}
      <div className="glass-panel" style={{ padding: '24px', marginBottom: '24px', background: 'rgba(16, 185, 129, 0.05)', borderColor: 'rgba(16, 185, 129, 0.2)' }}>
        <h3 className="font-display" style={{ margin: '0 0 10px', fontSize: 16, fontWeight: 600, color: 'var(--accent-emerald)' }}>
          📅 30-Day Challenge
        </h3>
          ...
          {CHALLENGES.map((action, i) => (
            <label
              ...
              style={{
                ...
                background: selected.includes(action) ? 'rgba(16, 185, 129, 0.1)' : 'var(--glass-base)',
        ...
        {pledgeSaved ? (
          <div style={{
            marginTop: 16, padding: '14px 16px', background: 'rgba(16, 185, 129, 0.1)',
            border: '1px solid rgba(16, 185, 129, 0.3)', borderRadius: '12px',
            fontSize: 14, color: 'var(--accent-emerald)', fontWeight: 600, textAlign: 'center',
          }}>
            {authMode === 'firebase'
      ...
      {/* Architecture note */}
      <div className="glass-panel" style={{ padding: '24px', background: 'rgba(96, 165, 250, 0.05)', borderColor: 'rgba(96, 165, 250, 0.2)' }}>
        <h3 className="font-display" style={{ margin: '0 0 16px', fontSize: 13, textTransform: 'uppercase', letterSpacing: '0.1em', color: 'var(--accent-blue)', fontWeight: 700 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          {[
            ...
            <div key={svc} style={{
              padding: '12px', background: 'var(--glass-base)',
              borderRadius: '12px', border: '1px solid var(--glass-border)',
            }}>
              <p style={{ margin: 0, fontSize: 12, fontWeight: 600, color: 'var(--accent-blue)' }}>{svc}</p>
              <p style={{ margin: '4px 0 0', fontSize: 11, color: 'var(--text-secondary)', lineHeight: 1.4 }}>{desc}</p>
            </div>
          ))}
        </div>
      </div>
